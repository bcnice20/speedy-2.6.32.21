...
static void curcial_oj_shutdown(int enable)
{
	...
	if (enable) { /*enter early suspend*/
		...
		msm_proc_comm(PCOM_RPC_GPIO_TLMM_CONFIG_EX, &id, 0);
		id = PCOM_GPIO_CFG(VISION_OJ_MOTION, 0, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_2MA);
		msm_proc_comm(PCOM_RPC_GPIO_TLMM_CONFIG_EX, &id, 0);
		gpio_set_value(VISION_OJ_MOTION, 0);
		...
		id = PCOM_GPIO_CFG(VISION_OJ_MOTION, 0, GPIO_INPUT, GPIO_NO_PULL, GPIO_2MA);
		msm_proc_comm(PCOM_RPC_GPIO_TLMM_CONFIG_EX, &id, 0);
	}
